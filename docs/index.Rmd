...
onde $L$ é operador $\textit{backward}$, $\phi_0 = -1 = \theta_0$ e assumimos que $\phi(\cdot)$ e $\alpha(\cdot)$ não possuem raízes em comum e $\epsilon_t$ é um erro aleatório. 
...
em que $\boldsymbol{\Theta}\subseteq \{\mathbb{R}_+^5\times \mathbb{R}^{k+1+p+q}\}$ é o espaço de parâmetros e $\ell(\boldsymbol{\theta})$ é a função de log-verossimilhança de $\boldsymbol{y}$ é dada por
...
$$\boldsymbol{\theta}=(\alpha,\sigma,\gamma,\eta,\lambda,\boldsymbol{\beta}^\top,\boldsymbol{\phi}^\top, \boldsymbol{\theta}^\top)^\top$$
