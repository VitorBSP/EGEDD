...
A estimação dos parâmetros será realizada através de Estimadores de Máxima Verossimilhança (EMV) e a máxima $L_q$-verossimilhança $(ML_qE)$, com o vetor de parâmetros:
...
A estimação do vetor de parâmetros $\boldsymbol{\theta}$ através do método da máxima verossimilhança, denotado por EMV, é dado por
 
$$\widehat{\boldsymbol{\theta}}=argmax_{{\boldsymbol{\theta}}\in \Theta}\{\ell(\boldsymbol{\theta})\}$$
--

em que $\boldsymbol{\Theta}\subseteq \{\mathbb{R}_+^5\times \mathbb{R}^{k+1}\}$ é o espaço de parâmetros e $\ell(\boldsymbol{\theta})$ é a função de log-verossimilhança de $\boldsymbol{y}$ é dada por
$$\ell(\boldsymbol{\theta})=\ell(\boldsymbol{\theta};\boldsymbol{y})=\sum_{t=1}^{n}\ell_t(\boldsymbol{\theta},{{y_t}}),$$

--

em que
$$\ell_t(\boldsymbol{\theta},y_t) = log(\alpha\delta\sigma\gamma\eta\lambda)-(\delta+1)\log(y_t)-(\sigma+1)\log(z_t)$$
...
Considerando a reparametrização por $\delta:$
...
o vetor de parâmetros do modelo proposto é dado por
$$\boldsymbol{\theta}=(\alpha,\sigma,\gamma,\eta,\lambda,\boldsymbol{\beta}^\top)^\top$$
...
---
```{r}
knitr::include_app("https://vitorpereira.shinyapps.io/EGEDD_FDP/", height = '600px')
