...
- Nota-se que nesses artigos, tem-se uma reparametrização em termos da média ou mediana, para possibilitara interpretação dos parâmetros em termos de medidas de posição ou precisão; 
...
$$\phi(L)=-\sum_{j=0}^{p}\phi_jz^j,\quad \theta(L)=-\sum_{j=0}^{q}\theta_jz^j$$
---
```{r}
